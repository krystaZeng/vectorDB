...
@Component
public class VectorEngineDataRouter {

    private final Map<String, VectorEngineDataPort> portsByEngineType;

    public VectorEngineDataRouter(List<VectorEngineDataPort> ports) {
    ...
    public VectorEngineDataPort get(String engineType) {
        String normalized = normalize(engineType);
        VectorEngineDataPort port = portsByEngineType.get(normalized);
        if (port == null) {
            throw new BizException("vector engine data port is not supported: " + normalized);
        }
        return port;
    }
